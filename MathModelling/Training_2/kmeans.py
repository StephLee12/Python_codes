...
class KMeans:
    ...
    def k_initialize(self):
        ...
        for _ in range(self.cluster_num):  #获取k个初始值
            ...
            for j in range(len(min_max)):
                # 将随机树保留4位小数
                rand_num = round(random.uniform(min_max[j][0], min_max[j][1]),
                                 2)
                tmp[j] = rand_num
            init_val.append(tmp)
    ...
    def kmeans_main(self):
        ...
        while iteration <= self.iteration and flag == 0:
            ...
            for idx in range(self.cluster_num):
                ...
                data = self.data.df[self.data.df[self.data.cluster_column] ==
                                    idx]
                tmp_count = 0
                for col_name in self.data.fea_column:
    ...
    def show_res(self):
        ...
        for idx in range(self.cluster_num):
            ...
            # 获取类别
            label = str(df[self.data.class_column].mode().get(0),
                        encoding='utf-8')
            cluster_size.append('cluster ' + str(idx) + ' ' + label +
                                '\'s size:' + df_size)
            ax = df.plot.scatter(x=self.data.fea_column[0],
