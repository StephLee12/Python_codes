    ...
    for column in columns:
        if column not in nominal_column:#如果是numeric类型
            for i in range(df.shape[0]):
                #处理缺失值
                if math.isnan(df.iat[i,column_count]):#df.iat[i,column_count].isnan():
                    ...
                    for j in first_index:
                        if math.isnan(df.iat[j,column_count]): #若出现对应的值是nan的情况
                            counter_1 += 1
                            continue
                    ...
                    if mean == 0: #最初的五个相似度最高的均为nan
                        ...
                        while mean == 0 and count < 53:
                            ...
                            if counter_2 == 5: #避免除0的情况出现
                                ...
                                count += 1
                                df.iloc[i,column_count] = mean /(5-counter_2)
                    else:
        ...
        for j in range(len(sim_list)):
            ...
            for k in range(mat.shape[1]):
                if k not in nominal_index:
                    #若是数值属性
                    if ((obj_1[:,k] == b'nan') or (obj_2[:,k] == b'nan')):  #若某个对象的k属性不存在
                        continue
                    else:
...
def get_simlist_max(simlist,k):
    index = sorted(range(len(simlist)) , 
                key = lambda sub: simlist[sub])[-k:]
    index.reverse()
    return index
